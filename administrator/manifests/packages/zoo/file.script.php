...
class pkg_zooInstallerScript extends ZooInstallerScript {
    ...
    public function postflight($type, $parent, $results) {
        ...
        if ($db->getNumRows() == 0) {
            ...
            // update module positions if YOOtheme Pro template
            $tmplParams = $db->setQuery("SELECT `params` FROM #__template_styles WHERE client_id=0 AND home=1")->loadResult();
            if (json_decode($tmplParams)->yootheme) {
                $db->setQuery("UPDATE `#__modules` SET `position` = 'sidebar' WHERE `position` = 'sidebar-b' AND `module` LIKE 'mod_zoo%'")->execute();
            }
