	...
	*/
	...
	$.fn.boxplusCarousel = function (settings) {
		...
		var defaults = {
			...
			scrollBy: 1,                // the number of items to scroll in a single step
		...
		lists.each(function () {
			...
			function scroll(dirmag) {
				if (!dirmag) {
					return;
				}

				var movingitem = listitems[dirmag > 0 ? 'first' : 'last']();  // item that is relocated in the HTML DOM
				var dir = dirmag > 0 ? 1 : -1;
				...
				ribbon.animate(target, duration, settings.scrollBy > 1 ? 'linear' : 'swing', function () {
